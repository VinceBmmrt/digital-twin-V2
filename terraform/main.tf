...
# CloudFront reaches the bucket via Origin Access Control (below), not the S3
# website endpoint, so the bucket itself stays private end to end.
...
# Least-privilege replacement for AmazonBedrockFullAccess: only allow
# invoking models/inference profiles, not model management, tuning, etc.
...
# Least-privilege replacement for AmazonS3FullAccess: scoped to this app's own
# conversation-memory bucket only, not every bucket in the account.
...
# Origin Access Control lets CloudFront read the private frontend bucket
# directly over HTTPS, instead of going through the public S3 website
